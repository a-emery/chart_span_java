function convert() {
    ...
    var answer = 0;
    for(i = 0; i < ipArray.length; i++) { // Go into each chunk of the ipAddress
        var number = ipArray[i]; // Store the value of a chunk to a variable that can be manipulated
        binaryArray[i] = []; // Create an index in the binaryArray to store data about this chunk
        for(j = 7; j >= 0; j--) {
            if(number >= Math.pow(2, j)) { // If number is larger than 2^j, add a 1 to that chunks index of the binaryArray and subtract 2^j from number
                binaryArray[i].push(1);
                number = number - Math.pow(2, j);
            } else { // Otherwise, add a 0 and keep numer as it
